...
**Re-verification required before Kaggle re-upload:** the previous "fully verified as of 2026-07-18" declaration (§1.4.2) was against the pre-fix, alpha-only-masked data. This regeneration has been verified quantitatively (mask coverage now realistic dataset-wide) and spot-visually (`Italy_022`, `Italy_004` in `notebooks/find_corrupted_masks.ipynb`), but the full `verify_alignment_sanity_check.ipynb`-style visual review has not been re-run against the regenerated `aligned_raw/` — recommended before the next Kaggle training run, particularly for a sample of the 29 newly-fixed white-background patients beyond the 2 already spot-checked.
...
Once training moved to the cleaned aligned dataset (§1.4.3), a second candidate loss was added rather than assuming `BCEDiceLoss` is the right answer: **Focal Tversky Loss** (Abraham & Khan, 2018), `scripts/trainer_engine.py`. Tversky loss generalizes Dice with independent false-positive/false-negative weights (`α`, `β`); Focal Tversky adds a focusing exponent `γ`:
