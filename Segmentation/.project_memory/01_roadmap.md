...
- [ ] **In progress (started 2026-08-08 on real Kaggle T4×2):** the real 18-combo × 12-trial Kaggle sweep. Setup/clone/data-staging cells confirmed working on real Kaggle hardware — 201/201 palpebral + 211/211 forniceal_palpebral images+masks staged, matching local alignment totals exactly (see `.project_memory/kaggle/01_kaggle_notes.md`). Combined sanity-check cell and the 18 training cells themselves not yet confirmed. Also still not done: an individual end-to-end 1-trial/1-epoch dry run through the real `run_study()` path for each of the 9 builders specifically (only one representative model per integration path — SMP direct, custom TransUNet, HF adapter — was dry-run verified so far, per `04_pretrained_architecture_sweep.md`).
