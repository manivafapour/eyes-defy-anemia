...
**Update (2026-07-29): batch 1 confirmed complete** — see the "Batch 1 results downloaded" and "Batch 2 notebook created" sections further down this file for the full account.
...
All three findings above (missing `optuna` install, the shallow-path/duplicate data-copy cells, plus the nested-clone robustness fix) were rolled into a single regenerated notebook: **`classification/Kaggle-Notebook/classification-final-fixed.ipynb`** (later committed and pushed, commit `8c7c263`, once the project author explicitly asked). 24 cells total (4 markdown section headers + 20 code), in order: GPU check → `rm -rf` + clone + `%cd` → `/kaggle/input` listing (diagnostic) → `pip install -q optuna albumentations` → single clean data-copy cell (`shutil.rmtree(DST_DIR, ignore_errors=True)` before copying, replacing the old two-cell shallow-path-then-fix pattern — fully idempotent, no more stray `processed-dataset/` leftover) → dataloader sanity check → 14 batch-1 training cells (`regnet_y_400mf` → `mobilenet_v3_small` → `efficientnet_b0` → `resnet18` → `densenet121` → `convnext_tiny` → `swin_t`, each × palpebral then forniceal_palpebral, cheapest first). Validated two ways before confirming: parsed as JSON, and passed `nbformat.validate()` (strict schema check, not just "is it valid JSON") — no cells have been executed (this file was generated, not run; `execution_count: null` and empty `outputs` on every code cell, honestly reflecting that).

## Batch 1 results downloaded: zip carries v1 "passenger" files (2026-07-29)

Real, useful finding for interpreting any future batch's zip: `sync_outputs()` (both batches) copies whatever is physically present under `classification/outputs/{checkpoints,logs,plots}/` at call time — and since the Kaggle notebook does a fresh `git clone` of the full repo, any already-git-tracked content in those folders (the old v1 `logs/`/`plots/`, which are tracked; `checkpoints/` is gitignored so this doesn't apply there) is present from the moment of cloning, *before* any training happens. So `batch1_results.zip`'s `logs/`/`plots/` contained 40/74 files respectively — the expected 14×2/14×4 new v2 entries, plus 12/18 old v1-named entries carried along unchanged. `checkpoints/` had exactly 14 files, no v1 passengers, since v1 checkpoints were never in the fresh clone to begin with.

**Practical implication for batch 2 and beyond:** the same thing will happen with `batch2_results.zip` once it's downloaded — except this time the "passengers" will be whatever's sitting in `classification/outputs/` in the *committed* state at that point, which (depending on whether batch 1's v2 results get committed before batch 2 runs) could include the batch-1 v2 results too, not just old v1 ones. Worth checking the zip's contents (`zipfile.namelist()`, no need to extract first) before assuming what's in it, same discipline used for batch 1.

## Batch 2 notebook created (2026-07-29)

New, standalone notebook `classification/Kaggle-Notebook/classification-batch2.ipynb` — the 4 remaining combos (`vit_b_16`, `vit_l_16` × palpebral/forniceal_palpebral), the two heaviest architectures in the 9-architecture roster (86.6M / 304.3M params), deliberately kept in their own dedicated session per the original batching decision above. Not a modification of `classification-final-fixed.ipynb` — generated by transforming it programmatically (loading its JSON, reusing the already-Kaggle-verified Setup/Data cells verbatim, swapping in 4 new training cells, retargeting `sync_outputs()` at `batch2_results.zip`). 17 cells total. Not yet run on Kaggle, not yet committed to git.

Caught one real bug during generation, before declaring it done: the transformation script's cell-slice boundary was off by one and carried over the stale "## Training -- Batch 1" markdown header alongside the new "## Training -- Batch 2" one. Found by inspecting the full cell list after generation, not by trusting the script ran without error — fixed, regenerated, re-validated with `nbformat.validate()`, and all 4 referenced `train_{vit_b_16,vit_l_16}_{palpebral,forniceal_palpebral}_v2.py` scripts confirmed to exist on disk.
