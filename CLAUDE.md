...
A `cv2.selectROI`-based manual bounding-box annotation tool was built and tried for the 15 failed patients. **This was rejected** in favor of completely dropping these 15 edge-case patients from the segmentation pipeline: forcing an alignment onto extreme edge cases risked introducing noisy training targets (a hand-drawn rectangle is a much cruder ground truth than the SIFT/RANSAC-derived mask the other 202 patients have, and would be inconsistent within the same dataset). The manual-annotation script was deleted. **The final segmentation training dataset is therefore exactly 202 patients** — chosen deliberately to maintain pristine, consistently-derived data quality over maximizing patient count.
...
Once training moved to the cleaned, 202-patient aligned dataset (§1.4.3), a second candidate loss was added rather than assuming `BCEDiceLoss` is the right answer: **Focal Tversky Loss** (Abraham & Khan, 2018), `scripts/trainer_engine.py`. Tversky loss generalizes Dice with independent false-positive/false-negative weights (`α`, `β`); Focal Tversky adds a focusing exponent `γ`:

`TI = (TP + ε) / (TP + α·FP + β·FN + ε)`, `FTL = (1 − TI)^γ`

Defaults `α=0.3, β=0.7, γ=4/3` (the original paper's values). `β > α` penalizes false negatives more than false positives — every missed true-foreground pixel is a false negative, so this directly targets the all-background collapse mode. `γ > 1` down-weights already-easy samples, concentrating gradient on poorly-segmented ones — relevant here because the 202-patient set's foreground fraction is **bimodal**, not uniformly sparse (measured: median 4.15%, 75th percentile 5.71%, but a small Italy-crop cluster sits at 75.0% — 95th percentile jumps straight from single digits to 75%). Being computed per-sample as a ratio, both Tversky and Dice adapt automatically to each image's own sparsity; a global `BCEWithLogitsLoss(pos_weight=...)` cannot (a `pos_weight` tuned for the ~4% typical case would badly over-penalize the 75%-foreground cluster, where background is the minority).
