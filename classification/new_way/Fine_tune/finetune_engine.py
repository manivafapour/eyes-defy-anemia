"""
Partial fine-tuning: unfreeze block 3 (the last block) of ConvNeXt-Base's
stage 4, continuing training from the already-converged, frozen-backbone
checkpoint (Output/version1/checkpoints/best_convnext_base_palpebral_new_way.pth,
val F1=0.9333) rather than starting from a fresh head. This is genuine
fine-tuning -- picking up from where the original Optuna search left off
and giving the model a small amount of extra backbone capacity to adapt --
not a from-scratch retrain with one layer unfrozen.

Why block 3 alone, not the full last stage: discussed and decided in chat
before any code was written. features[7] (stage 4) has 3 identical
CNBlocks, 8.45M params each. Block 3 alone gives ~3x fewer trainable
backbone params than unfreezing all of stage 4, which matters a lot on
this project's small dataset (~151 train patients for this single-split
protocol) -- both for overfitting risk and, more specifically to this
project's own documented concern (classification/.project_memory/
07_step1_measurement_harness.md, Step 5), for how much room the model
gets to encode the India/Italy shortcut instead of genuine pallor signal.
ConvNeXt has zero BatchNorm anywhere (verified by inspection) -- so the
frozen-but-still-updating-running-stats gotcha that would matter for
EfficientNet-B3/RegNetY-16GF does not apply here.

Data: TRAIN reads new_way/balanced_dataset.py's BalancedTissueClassification
Dataset in its default (single-split) mode -- patient_ids=None, which
filters to the original "train" split's patients (see that module's
docstring for why the augmented-image count is close to but not exactly
50/50 per country after that filter: 47/47 India, 69/67 Italy for
palpebral, verified, not a bug). VAL and the sealed TEST are always real,
unmodified, unaugmented images, matching every other pipeline in this
project. This is a SINGLE train/val/test split, not the 3-fold CV protocol
built in new_way/kfold/ -- deliberately kept separate and simpler, since
this is a first pilot to see whether partial unfreezing helps at all
before deciding whether it's worth the extra k-fold compute.

Discriminative learning rates (judgment calls, flagged): the head
continues at its original best-trial LR, and block 3 gets a further 1/10
of that. weight_decay and dropout_rate are reused unchanged from the
original best trial. MAX_EPOCHS is deliberately shorter than the original
250-epoch search -- this is a refinement phase on an already-converged
model, not a from-scratch search. Scheduler and early-stopping track
val_f1, not val_loss -- see the run_finetune() docstring below for why.

Train loop and plotting are shared with every other per-model fine-tune
engine in this directory via finetune_common.py -- only build_finetune_model()
(architecture-specific: which submodules to unfreeze) and this file's own
run_finetune() (per-model hyperparameter/param-group setup) are not.
"""
...
# Configuration
# --------------------------------------------------------------------------
MODEL_NAME = "convnext_base_palpebral_new_way_finetune_block3_v2"  # v2: higher LR + val-F1-tracked
# scheduler/early-stopping, distinct from the first pilot's outputs (kept, not overwritten) --
# see the HEAD_LR_FACTOR/BLOCK3_LR_FACTOR comment above and the scheduler/early-stopping change below.
TISSUE_TYPE = "palpebral"
IMAGE_SIZE = 256

SOURCE_CHECKPOINT = NEW_WAY_DIR / "Output" / "version1" / "checkpoints" / "best_convnext_base_palpebral_new_way.pth"

# Original best trial (Output/version1/logs/convnext_base_palpebral_new_way_study_summary.json)
...
ORIGINAL_LEARNING_RATE = 0.0001235308191492332
ORIGINAL_WEIGHT_DECAY = 1.0422971466648463e-06
ORIGINAL_DROPOUT_RATE = 0.5

HEAD_LR_FACTOR = 1.0  # head continues at its original best-trial LR -- raised 5x (was 0.2) after the
# first pilot run showed the model barely moved in 60 epochs at the old rate: val_f1 tied baseline
# after epoch 1 then only got worse, while val_loss kept improving the whole time (see
# Output/logs/*_history.json from that run). BLOCK3_LR_FACTOR unchanged, so block3_lr scales up by
# the same 5x automatically, preserving the intentional 10x head:block3 ratio.
...
def build_finetune_model(dropout_rate: float = ORIGINAL_DROPOUT_RATE, checkpoint_path: Path = SOURCE_CHECKPOINT) -> nn.Module:
    ...
    # during the original training too (trainer_engine.py's _freeze_all()
    # runs before classifier[2] gets replaced) and must stay frozen here for
    # consistency. Unfreezing the whole `model.classifier` Sequential would
    # silently also unfreeze that LayerNorm2d -- caught by a real parameter-
    # count check before this was trusted (see verification notes).
...
def run_finetune(
    ...
    """mode="max" on val_f1, not val_loss, for both the scheduler and early
    stopping -- the first (v1) pilot run showed the two diverging (val_loss
    kept improving for all 60 epochs while val_f1 got steadily worse after
    epoch ~2), which meant a val_loss-tracked scheduler/early-stopping never
    reacted to the thing that actually matters here. Every other pipeline in
    this project tracks val_loss because it trains from scratch, where loss
    and the target metric move together; this is a continuation from an
    already-converged point, where they can decouple."""
    ...
    # --- Baseline: evaluate the just-loaded (pre-fine-tune) model on val
    # and test BEFORE any fine-tune training happens, using the exact same
    # data pipeline and metric code as everything below -- an apples-to-
    # apples "before" number, not a historical figure from a different run. ---
