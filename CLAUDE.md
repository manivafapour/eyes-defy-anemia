...
**Session-start instruction:** before doing anything else in a new session, silently read `Segmentation/.project_memory/01_roadmap.md`, `Segmentation/.project_memory/02_current_status.md`, and `Segmentation/.project_memory/03_tech_stack_and_rules.md` to load current project context. After completing a major task or making an architectural decision, update the relevant file(s) in `Segmentation/.project_memory/` before moving on. `Segmentation/.project_memory/` is the lightweight, frequently-updated working-memory system; this file (`CLAUDE.md`) remains the authoritative, thesis-grade methodology record — the two should stay consistent, but `CLAUDE.md` is the one held to scientific-accuracy standards.

## Repository & Security Rules (standing, applies to every session)

**Never push or track any private files, raw datasets, or research source directories to GitHub.** Specifically, the entire `Source/` directory (local literature PDFs, internal research summaries, and any other local-only documents) is covered by `.gitignore` at the repo root and must never be staged or committed, regardless of what subfolder structure gets added under it later. Before any broad `git add`, confirm with `git status` that nothing under `Source/` (or any other private/local-only directory) is about to be staged — don't rely on `.gitignore` alone as the only safeguard. This rule was added 2026-07-28 after the project author placed literature-review source material (PDFs, a research summary docx) in `Source/Paper_Eye/`; verified at the time that `Source/` had never previously appeared in git history (confirmed via `git log --all -- Source/`, empty) before the ignore rule was added.
...
## Phase 0 — Data Engineering (`Segmentation/scripts/phase0_prepare_dataset.py`)
...
The raw photo is saved to `Segmentation/data/processed/images/{patient_id}.jpg`; the palpebral crop is saved to `Segmentation/data/processed/masks/{patient_id}_palpebral.png`. **Important:** these two files are padded/resized independently of one another, so they do **not** share a common pixel coordinate grid (see §1.2 for why this matters for segmentation).
...
## Phase 1 — Data Pipeline Construction (`Segmentation/scripts/dataset.py`)
...
Verified realized split (`Segmentation/data/processed/dataset_splits.csv`):
...
This dataset returns `(image, mask)` pairs, both sourced from the **same** file, `Segmentation/data/processed/masks/{patient_id}_palpebral.png`:
...
### 1.4 Data-centric fix: raw-photo-aligned segmentation dataset (`Segmentation/scripts/build_aligned_dataset.py`)
...
The first implementation located the crop inside the raw photo via `cv2.matchTemplate` (`TM_CCORR_NORMED`, masked by the crop's own alpha channel). It reported excellent-looking aggregate statistics — 217/217 "aligned," confidence range 0.952–0.998 — and passed 5 hand-picked visual spot-checks. **It was still wrong.** A later manual review of `India_071` (not one of the 5 originally spot-checked) found the "aligned" mask floating on the sclera (white of the eye) instead of the lower eyelid — a complete, spatially-wrong placement, despite a 0.995 confidence score. The lesson: `matchTemplate` is a pure appearance-correlation score with no geometric constraint, so a high score never guaranteed a correct location — it can lock onto a *different* region that coincidentally correlates well (similar skin tone, vessel texture, lighting). The "is the mask blank" automated sanity check from this version (`Segmentation/notebooks/verify_alignment_sanity_check.ipynb`, first version) could not catch this failure mode at all, since the mask was very much non-blank — just in the wrong place. **This entire approach, and the `aligned_raw` data it produced, was discarded.**

**Consequence for Phase 2 training:** an early Kaggle run trained against this v1 data collapsed to an all-background prediction (`val_dice` pinned at `0.0000` while `val_loss` kept decreasing, §3.2a). A model cannot learn a meaningful image→mask relationship from targets that are non-blank but spatially decoupled from the true tissue location — training on mathematically-valid, anatomically-wrong masks (e.g. sclera instead of palpebral conjunctiva) is a textbook "garbage in, garbage out" scenario. This v1 data quality problem is therefore judged the **most likely root cause** of that collapse. This has not been independently proven as the *sole* cause (as opposed to, or in combination with, the loss function's own weighting, §3.2a) — §3.2b's side-by-side loss comparison on the clean v2 data was built partly to help settle this.
...
7. Output: `Segmentation/data/processed/aligned_raw/images/{patient_id}.jpg`, `Segmentation/data/processed/aligned_raw/masks/{patient_id}.png`, plus `alignment_log.csv` (per-patient method, keypoint/match/inlier counts).

**Results (full 217-patient run, original alpha-only masking):** 202/217 aligned with high inlier confidence — **15 honest failures** (rejected by the geometric sanity checks above, rather than silently producing a wrong result): 13 "corners outside raw photo bounds," 2 "area ratio outside bounds." All 202 successes used SIFT (ORB fallback was never the better candidate). Inlier counts range 5–2014 (mean 311); 9 flagged low-confidence (<15 inliers). Of the produced masks, 0/202 are blank or near-blank (min positive pixels 1793). **Superseded by §1.4.4 below** — a subsequent bug fix to the mask-extraction logic itself changed this to 201/217 (one of the 202 turned out to be a false-positive success, not a new failure); the current, correct counts are in §1.4.4.

**Why the 15 likely failed:** a follow-up visual review (`Segmentation/notebooks/investigate_alignment_failures.ipynb`, raw photo + original crop side by side at native resolution, no alignment attempted) spot-checked a subset of the 15 and found two plausible, distinct failure modes — consistent with, but not an exhaustive confirmation across, all 15 patients: (a) the 2 area-ratio failures (e.g. `India_060`) show a visibly thinner, lower-area tissue strip than successfully-aligned patients sharing the same crop dimensions, plausibly leaving too few distinctive SIFT keypoints for a stable fit; (b) the 13 corners-outside-bounds failures are consistent with a more severe crop-to-raw scale mismatch (an even larger zoom-level difference than the ~3.7× ratio the sanity bound above was tuned to accept) than RANSAC could resolve into a geometrically valid homography. A full, systematic per-patient classification of which cause applies to which of the 15 has not been performed. (A 16th failure, `Italy_026`, was added later for an unrelated reason — see §1.4.4.)

**Visual verification:** `India_071` (the patient that exposed v1's failure) now shows the mask precisely on the lower eyelid, matching the original crop's shape. Additional spot-checks (`India_001`, `India_029`, `Italy_097`) also show correct, anatomically-sensible placement. **Dataset declared fully verified as of 2026-07-18:** the project author personally reviewed `Segmentation/notebooks/verify_alignment_sanity_check.ipynb`'s full output (all 10 sampled patients, including `India_071`, plus the full-dataset failure list) and confirmed it — this was the explicit gate blocking training, and it is now cleared.

**New dataset class:** `AlignedConjunctivaSegmentationDataset` (`Segmentation/scripts/dataset.py`) reads `(image, mask)` from `Segmentation/data/processed/aligned_raw/{images,masks}/` — image is the *full raw photo*, mask is a genuinely pixel-aligned tissue mask in that same frame. It is additive, not a replacement: `ConjunctivaSegmentationDataset` (§1.2) is unchanged and still valid for its original purpose (and for comparison); `AlignedConjunctivaSegmentationDataset` is the dataset to use going forward for a segmentation model intended to generalize to raw photos. Both are wired into `get_dataloaders()` (`seg_*` vs. `aligned_seg_*` keys) using the same `get_train_transforms()`/`get_eval_transforms()` pipeline (§1.3) — no new transform logic was needed, since both datasets share the same 256×256 RGB image / single-channel binary mask contract.

#### 1.4.3 Decision: drop the 15 failed patients rather than manually annotate them
A `cv2.selectROI`-based manual bounding-box annotation tool was built and tried for the 15 failed patients. **This was rejected** in favor of completely dropping these 15 edge-case patients from the segmentation pipeline: forcing an alignment onto extreme edge cases risked introducing noisy training targets (a hand-drawn rectangle is a much cruder ground truth than the SIFT/RANSAC-derived mask the other patients have, and would be inconsistent within the same dataset). The manual-annotation script was deleted.

`AlignedConjunctivaSegmentationDataset` filters to only patients with `status == "ok"` in `alignment_log.csv`, joined against `dataset_splits.csv` **without modifying that shared CSV** — `ConjunctivaSegmentationDataset` and `AnemiaClassificationDataset` still see all 217 patients, since the excluded patients here are perfectly valid data for those.

**Numbers as of this section (original alpha-only masking, since superseded — see §1.4.4):** 202 patients, `aligned_seg_train`/`val`/`test` = 143/28/31, 15 excluded (13 India, 2 Italy). Skewed the aligned-only subset's country balance toward Italy relative to the original 70/15/15 stratification (§1.1) (e.g. train: India 66→59, Italy 85→84).

**Update:** `Segmentation/scripts/trainer_engine.py` now accepts a `dataset_cls` parameter (§2.6, §3.2), and three new sibling entry-point scripts (`train_standard_unet_aligned.py`, `train_attention_unet_aligned.py`, `train_resunet_aligned.py`) target `AlignedConjunctivaSegmentationDataset`. The original three entry-point scripts are untouched and still train against `ConjunctivaSegmentationDataset` by default. Getting `Segmentation/data/processed/aligned_raw/{images,masks}/` onto Kaggle (they're `.gitignore`'d, so `git pull` alone won't bring them over) and updating the Kaggle-side copy/setup script are separate steps, done manually by the project author outside this repo.

#### 1.4.4 Bug fix: white-background source convention was silently miscounted as foreground
**Symptom, caught by manual review:** inspecting individual aligned masks (not just aggregate stats) found that a subset of Italy patients (e.g. `Italy_022`) had a mask covering ~75% of the 256×256 frame — the *entire* visible eye (iris, pupil, sclera, skin), not the palpebral conjunctiva. This had gone undetected because it presented as a plausible-looking aggregate statistic rather than an obvious error: the per-patient foreground-fraction distribution *looked* bimodal (§3.2b, before this fix, incorrectly described a "small Italy-crop cluster at 75%" as if it were a real anatomical phenomenon) rather than visibly broken.

**Root cause:** a subset of patients' *source* palpebral crop PNGs in `archive.zip` delimit tissue with an **opaque white (255,255,255) background** instead of alpha transparency — a different convention from the rest of the dataset, not missing or corrupted data. `build_aligned_dataset.py`'s original `(alpha > ALPHA_THRESHOLD)` mask logic silently mis-evaluates this as "the entire rectangle is foreground," since alpha is 255 everywhere regardless of what the RGB shows. This is a source-data convention difference, not a SIFT/RANSAC bug — verified by reading the affected patients' original crop PNGs directly, before any alignment code runs at all: the tissue is clearly, unambiguously visible in the RGB channels in every case, just not encoded via alpha.

**Detection is automatic, not a hardcoded patient list** (`_alpha_is_functional()`): a crop's alpha channel is treated as functional only if less than `OPAQUE_ALPHA_FRACTION_THRESHOLD = 0.99` of it is opaque; a crop with no alpha channel at all (plain RGB) is treated identically to a uniformly-opaque one (Pillow's `.convert("RGBA")` would otherwise silently manufacture full opacity for it, hiding the exact same problem). This threshold was chosen from a full 217-patient scan of the *source* data, not guessed: legitimate alpha-cutout patients top out at 13.0% opaque fraction (`Italy_113`); every affected patient sits at exactly 100% — any threshold between those two clusters is safe, with a wide margin either way.

**Fallback mask (`_white_background_mask()`):** for patients whose alpha is non-functional, the foreground mask is instead built from `NOT near-white RGB` (any channel `< WHITE_BACKGROUND_THRESHOLD = 245`). This mask is used identically to how the alpha-derived mask was already used — for SIFT/ORB keypoint restriction *and* as the shape warped through the estimated homography into the raw photo's frame — so `find_homography_alignment` (the actual SIFT/RANSAC/geometric-sanity-check logic) required zero changes; only the mask *source* is now convention-aware.

**Verified, not assumed:** initial diagnosis (`Segmentation/notebooks/find_corrupted_masks.ipynb`) wrongly concluded from the opaque-alpha finding alone that these patients had no usable ground truth at all and should be dropped — the project author caught this by manually inspecting a source crop and finding the tissue clearly visible. The corrected fix was then verified two ways: (1) targeted, on `Italy_022`/`Italy_004`, mask coverage dropped from 75.00% to 2.68%/4.81% with RANSAC inlier counts unchanged (271/296), confirming only the mask *content* was ever wrong, not the geometry; (2) full regeneration, re-running `build_aligned_dataset.py` against all 217 patients with the fix in place.

**Full-regeneration results:** 29 patients now use the white-background fallback successfully (`Italy_001–007, 009–025, 027–031`, all previously miscounted at ~75%); one patient (`Italy_009`) had no alpha channel at all and is included in this count. **201/217 patients aligned** (was 202/217 under the old, buggy logic) — the count is *lower*, not higher, because `Italy_026` (previously the weakest of the corrupted 30, 28 inliers) was a **false-positive success** under the old permissive whole-rectangle mask and now honestly fails its geometric sanity check ("corners outside raw photo bounds") once the keypoint search is correctly restricted to just the tissue region — exactly the "honest failure over silently-wrong success" behavior this whole pipeline is designed around (§1.4.1, §1.4.2). Updated failure count: **16** (14 "corners outside bounds," 2 "area ratio outside bounds"; 13 India + 3 Italy). Mask coverage across all 201 successes is now a clean, unimodal 1.78%–9.77% (mean 4.05%, median 3.80%) — no outliers above 10%, no bimodal cluster. `alignment_log.csv` gained a `mask_source` column (`alpha` or `white_bg`) recording which convention each patient used, including for failures.

**Updated dataset size: 201 patients** (172 via the original alpha-cutout convention + 29 via the white-background fallback), `aligned_seg_train`/`val`/`test` = 143/27/31 (only `val` changed, by the one patient — `Italy_026` was a `val`-split patient). `ConjunctivaSegmentationDataset`/`AnemiaClassificationDataset` remain unaffected at 151/33/33 (217 total), as before. Country breakdown of the 16 excluded patients: 13 India, 3 Italy.

**Re-verification required before Kaggle re-upload:** the previous "fully verified as of 2026-07-18" declaration (§1.4.2) was against the pre-fix, alpha-only-masked data. This regeneration has been verified quantitatively (mask coverage now realistic dataset-wide) and spot-visually (`Italy_022`, `Italy_004` in `Segmentation/notebooks/find_corrupted_masks.ipynb`), but the full `verify_alignment_sanity_check.ipynb`-style visual review has not been re-run against the regenerated `aligned_raw/` — recommended before the next Kaggle training run, particularly for a sample of the 29 newly-fixed white-background patients beyond the 2 already spot-checked.
...
The model's final layer outputs **raw, unbounded logits** — no `Sigmoid` is applied inside the model. This is paired with one of two loss functions for the training objective, now selected per-trial as an Optuna hyperparameter (§3.2b): `BCEDiceLoss` (§3.2a, a combined `BCEWithLogitsLoss` + soft Dice loss) or `FocalTverskyLoss` (§3.2b) — both computed directly from logits. `BCEDiceLoss`'s `BCEWithLogitsLoss` half is a deliberate numerical-stability choice: it computes binary cross-entropy directly from logits using a log-sum-exp formulation (equivalent to `max(x,0) - x·z + log(1+exp(-|x|))`), which avoids the failure mode of a separate `Sigmoid` + `BCELoss` pipeline where `sigmoid(x)` can saturate to exactly `0.0` or `1.0` in floating point for large-magnitude logits, producing `log(0) = -inf` and NaN gradients. `torch.sigmoid()` is applied externally wherever an actual probability or binary mask is needed — at validation time (§3.3) and at inference time.
...
### 2.4 Model 2 — Attention U-Net (`Segmentation/models/segmentation/attention_unet.py`)
...
### 2.5 Model 3 — ResUNet (`Segmentation/models/segmentation/resunet.py`)
...
### 2.6 Model switching
All three models share an identical constructor signature (`Model(in_channels=3, out_channels=1)`) and forward contract (raw logits, `[B, 1, H, W]`). Training logic is not model- or dataset-specific: the shared engine (`Segmentation/scripts/trainer_engine.py`, §3) takes a model class/name *and* a dataset class as arguments (`dataset_cls`, defaulting to `ConjunctivaSegmentationDataset` for backward compatibility), and six thin entry-point scripts each just import the engine and call `run_study(model_cls=..., model_name=..., dataset_cls=...)` with their own architecture + dataset:

- `Segmentation/scripts/train_standard_unet.py`, `Segmentation/scripts/train_attention_unet.py`, `Segmentation/scripts/train_resunet.py` — original, crop-based `ConjunctivaSegmentationDataset` (§1.2), `model_name` = `unet`/`attention_unet`/`resunet`.
- `Segmentation/scripts/train_standard_unet_aligned.py`, `Segmentation/scripts/train_attention_unet_aligned.py`, `Segmentation/scripts/train_resunet_aligned.py` — raw-photo-aligned `AlignedConjunctivaSegmentationDataset` (§1.4), `model_name` = `unet_aligned`/`attention_unet_aligned`/`resunet_aligned`.

The `_aligned` suffix on `model_name` is deliberate, not cosmetic: every output path (`Segmentation/outputs/checkpoints/best_{model_name}.pth`, `Segmentation/outputs/logs/{model_name}_*`) is derived from `model_name`, so without a distinct name an aligned run would silently overwrite the existing crop-based checkpoints/logs. This replaced an earlier single-script `MODEL_REGISTRY`/`MODEL_NAME`-constant design — separate entry points were needed because execution happens on Kaggle, where editing a shared constant before each run isn't part of the workflow; a dedicated script per (model, dataset) pair can be run as-is.

---

## Phase 2 — Segmentation Training Procedure (`Segmentation/scripts/trainer_engine.py` + per-model entry points)
...
**Fix:** `criterion` is now `BCEDiceLoss` (`Segmentation/scripts/trainer_engine.py`) — `BCEWithLogitsLoss` averaged 50/50 with a soft (differentiable) Dice loss computed from `sigmoid(logits)` directly (not the hard-thresholded Dice used for the §3.3 metric, which can't be backpropagated through). Dice is a *ratio*, not a per-pixel average, so it stays scale-invariant to how small the true foreground is and keeps penalizing an all-background prediction heavily regardless.
...
**Scope of this change:** `trainer_engine.py` is shared by all six entry-point scripts (§2.6), so this affects future runs of the original crop-based scripts too, not just the `_aligned` ones — the crop-based dataset is less extremely imbalanced but not immune to the same failure mode. This means re-running `train_standard_unet.py` (etc.) today would **not** exactly reproduce the already-logged Kaggle results in `Segmentation/outputs/logs/{unet,attention_unet,resunet}_*` (§3.6), which were trained under plain `BCEWithLogitsLoss` — those logs remain a historical record of that specific run, not a currently-reproducible configuration.

**Reassessment:** this collapsed run was training against v1's anatomically-wrong masks (§1.4.1) — non-blank, but spatially decoupled from the true tissue location. That "garbage in" data is judged the **more likely explanation** for the collapse than insufficient loss weighting per se: a model has no learnable image→mask relationship to find when the target is consistently in the wrong place, regardless of how the loss balances false positives/negatives. This is not proven as the *sole* cause, since the two factors (bad masks, imperfect loss) were never tested independently — §3.2b's side-by-side design on the clean v2 (§1.4.2) data was built partly to help settle this properly.

### 3.2b `FocalTverskyLoss` and loss function as an Optuna hyperparameter
Once training moved to the cleaned aligned dataset (§1.4.3), a second candidate loss was added rather than assuming `BCEDiceLoss` is the right answer: **Focal Tversky Loss** (Abraham & Khan, 2018), `Segmentation/scripts/trainer_engine.py`. Tversky loss generalizes Dice with independent false-positive/false-negative weights (`α`, `β`); Focal Tversky adds a focusing exponent `γ`:

`TI = (TP + ε) / (TP + α·FP + β·FN + ε)`, `FTL = (1 − TI)^γ`

Defaults `α=0.3, β=0.7, γ=4/3` (the original paper's values). `β > α` penalizes false negatives more than false positives — every missed true-foreground pixel is a false negative, so this directly targets the all-background collapse mode. `γ > 1` down-weights already-easy samples, concentrating gradient on poorly-segmented ones.

**Correction (§1.4.4):** this section originally described the 202-patient set's foreground fraction as "bimodal... a small Italy-crop cluster sits at 75.0%." That was a measurement of a since-fixed bug (a subset of Italy patients' masks covered ~75% of the frame due to a mask-extraction fallback issue, not real anatomy) — see §1.4.4 for the full account. The corrected, current 201-patient set is **not bimodal**: foreground fraction ranges 1.78%–9.77% (mean 4.05%, median 3.80%, 95th percentile 6.75%) — sparse and right-skewed, but unimodal. `γ > 1`'s down-weighting of easy samples is still a reasonable design choice for a sparse segmentation target in general, but the original specific bimodal justification no longer holds; this has not been re-litigated against the corrected data. Being computed per-sample as a ratio, both Tversky and Dice adapt automatically to each image's own sparsity regardless; a global `BCEWithLogitsLoss(pos_weight=...)` would need a fixed value tuned for the typical ~4% case.

**Verified quantitatively** (same synthetic sparsity test as §3.2a, 130/65536 foreground): `FocalTverskyLoss` penalizes an all-background collapse at `0.9999` vs. `0.0013` for a correct prediction — an even larger relative gap than `BCEDiceLoss`'s `0.510` vs. `0.0057`. It also penalizes a *partial* miss (50% of true foreground pixels missed) far more heavily: `0.311` vs. `BCEDiceLoss`'s `0.177` for the identical partial prediction — confirming the `β=0.7` false-negative weighting has a real, larger effect than just the total-collapse case.

**Side-by-side comparison design:** rather than picking one loss upfront, `make_objective()`'s `objective(trial)` now samples `trial.suggest_categorical("loss_fn", ["bce_dice", "focal_tversky"])` alongside `learning_rate`/`weight_decay` — so a single Optuna study directly compares both across its trials, without any entry-point script changes (all six still just call `run_study(model_cls=..., model_name=..., dataset_cls=...)`; the loss choice is entirely internal to the shared engine now). `_save_outputs()` additionally saves a **per-loss-function checkpoint** (`Segmentation/outputs/checkpoints/best_{model_name}_{loss_fn}.pth`, alongside the existing single "best overall" checkpoint) and a **per-loss-function comparison table** (trial count, mean/max validation Dice per loss) in both the printed summary and `{model_name}_study_summary.json`'s `per_loss_fn_comparison` key — otherwise the single "overall best" checkpoint would hide whichever loss function didn't happen to win outright. Verified via a synthetic run (6 trials, mocked epoch results systematically favoring one loss) that both per-loss checkpoints are correctly written only from that loss's own trials, and the comparison table correctly attributes trial counts/means to each loss.

**Recommendation for interpreting results:** use `FocalTverskyLoss` standalone (not combined with BCE) if it wins — the `α`/`β` asymmetry already does what a BCE hybrid was trying to do, and stacking both adds tuning surface without a clear rationale here.

### 3.3 Validation metrics: sigmoid-threshold, then Dice/IoU
Because the model outputs raw logits (§2.2), validation applies `torch.sigmoid(logits)` followed by thresholding at `0.5` to obtain a binary predicted mask, *before* computing spatial overlap metrics (the loss itself, `BCEDiceLoss` or `FocalTverskyLoss` §3.2a/§3.2b, still consumes the raw logits directly — only the metrics operate on the thresholded binary mask). For each validation batch:
...
### 3.4 Execution plan
`optuna.create_study(direction="maximize", sampler=TPESampler(seed=42))` followed by `study.optimize(objective, n_trials=N)`. `run_study()`'s `N_TRIALS` engine default remains 5 (the original smoke-test budget, still used by the three original crop-based entry-point scripts, matching the already-logged §3.5/§3.6 results). The three `_aligned` entry-point scripts (§2.6) now pass **`n_trials=12`** explicitly, overriding that default: with `loss_fn` added as a 3rd Optuna-tuned dimension (§3.2b) alongside `learning_rate`/`weight_decay`, 5 trials would give too little coverage to meaningfully compare `bce_dice` vs. `focal_tversky` (a 5-trial run could plausibly land 4-1 or 1-4 between the two by chance). This is a resource-budget decision (more trials = more Kaggle GPU time), made explicitly by the project author rather than assumed.
...
- **`Segmentation/outputs/checkpoints/best_{model_name}.pth`** — the state dict of whichever epoch, across *all* trials in the study (not just the final trial), achieved the highest validation Dice. Tracked via a `best_overall_dice` value held in the `make_objective()` closure, which persists across every trial Optuna runs against that closure instance.
- **`Segmentation/outputs/logs/{model_name}_trials.csv`** — the full per-trial record (`study.trials_dataframe()`: params, value, user attributes, state, duration) for every trial in the study.
- **`Segmentation/outputs/logs/{model_name}_study_summary.json`** — a compact summary of the winning trial (number, Dice, IoU, hyperparameters, checkpoint path, UTC timestamp).

`Segmentation/outputs/checkpoints/` is `.gitignore`'d (large binaries, regenerable by retraining); `Segmentation/outputs/logs/` is not — those files are small and are meant to be committed as part of the experimental record.
